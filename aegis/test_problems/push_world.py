...
class guiWorld:
    ...
    def draw(self, bodies, bg_color=(64, 64, 64, 0)):
        ...
        def my_draw_circle(circle, body, fixture):
            position = (
                self.screen_origin + body.transform * circle.pos
            ) * self.PPM
        ...
        points = [
            (
                self.PPM * (x + self.screen_origin[0]),
...
class end_effector:
    def __init__(
        ...
        # forceunit for circle and rectangle
        ...
        world.CreateFrictionJoint(
            bodyA=base, bodyB=self.hand, maxForce=2, maxTorque=2
        )
...
def make_1thing(
    ...
    link.CreateFixture(
        shape=linkshape, density=thing_density, friction=thing_friction
    )
...
def make_base(table_width, table_length, b2world_interface):
    ...
    base = world.CreateStaticBody(
        position=(0, 0), shapes=b2PolygonShape(box=(table_length, table_width))
    )
...
if __name__ == "__main__":
    ...
    # push8 example
    x = np.array([-3, -3, 100, (1 / 4) * np.pi, 4, 2.5, 100, (1 / 4) * np.pi])
    d = push_8D(x, 4, 4, 0, -4, -3, 0, 3, 0, draw=True)
    print(d)
